...
UGameLiftTestGameInstance::UGameLiftTestGameInstance() {
...
//This isnt called if you kill the process
void UGameLiftTestGameInstance::Shutdown() {
...
void UGameLiftTestGameInstance::Init() {
  Super::Init();

  GetWorld()->GetTimerManager().SetTimer(GetResponseTimeHandle, this, &UGameLiftTestGameInstance::GetResponseTime, 1.0f, true, 1.0f);
}

void UGameLiftTestGameInstance::SetCognitoTokens(FString NewAccessToken, FString NewIdToken, FString NewRefreshToken) {
...
void UGameLiftTestGameInstance::RetrieveNewTokens() {
  if (AccessToken.Len() > 0 && RefreshToken.Len() > 0) {
    ...
    if (FJsonSerializer::Serialize(RequestObj.ToSharedRef(), Writer)) {
      ...
      RetrieveNewTokensRequest->OnProcessRequestComplete().BindUObject(this, &UGameLiftTestGameInstance::OnRetrieveNewTokensResponseReceived);
      ...
      GetWorld()->GetTimerManager().SetTimer(RetrieveNewTokensHandle, this, &UGameLiftTestGameInstance::RetrieveNewTokens, 1.0f, false, 30.0f);
    }
  }
}

void UGameLiftTestGameInstance::OnRetrieveNewTokensResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
  if (bWasSuccessful) {
...
void UGameLiftTestGameInstance::GetResponseTime() {
  TSharedRef<IHttpRequest> GetResponseTimeRequest = HttpModule->CreateRequest();
  GetResponseTimeRequest->OnProcessRequestComplete().BindUObject(this, &UGameLiftTestGameInstance::OnGetResponseTimeResponseReceived);
...
void UGameLiftTestGameInstance::OnGetResponseTimeResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
